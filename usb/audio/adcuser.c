...
//              EP0Buf[0] = Mute;
        ...
        case AUDIO_VOLUME_CONTROL:
          switch (SetupPacket.bRequest) {
            case AUDIO_REQUEST_GET_CUR:
              *((PRE_PACK uint16_t *)EP0Buf) = VolCur;
              return (TRUE);
            case AUDIO_REQUEST_GET_MIN:
              *((PRE_PACK uint16_t *)EP0Buf) = VolMin;
              return (TRUE);
            case AUDIO_REQUEST_GET_MAX:
              *((PRE_PACK uint16_t *)EP0Buf) = VolMax;
              return (TRUE);
            case AUDIO_REQUEST_GET_RES:
              *((PRE_PACK uint16_t *)EP0Buf) = VolRes;
              return (TRUE);
          }
          break;
      }
...
//              Mute = EP0Buf[0];
        ...
        case AUDIO_VOLUME_CONTROL:
          switch (SetupPacket.bRequest) {
            case AUDIO_REQUEST_SET_CUR:
              VolCur = *((PRE_PACK uint16_t *)EP0Buf);
              return (TRUE);
          }
          break;
      }
