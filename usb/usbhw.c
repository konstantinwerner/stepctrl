...
uint32_t USB_GetEPCmdStatusPtr(uint32_t EPNum)
{
	...
	addr += ((EPAdr(EPNum) * 2) << 2);
	/* Based on EPCommand/Status List, all EPs are
...
void USB_Init(void)
{
	...
	NVIC_SetPriority(USB_IRQn, 1); // Lower than I2C
...
uint32_t USB_ReadEP(uint32_t EPNum, uint8_t *pData)
{
	...
	*addr &= ~0x3FFFFFF;
	if ((EPNum & 0x0F) == 0)
	{
		...
		*addr |= ((EPList[index].buf_length << 16) | (uint16_t) ((EPList[index].buf_ptr) >> 6) | BUF_ACTIVE);
	}
	else
	{
...
uint32_t USB_WriteEP(uint32_t EPNum, uint8_t *pData, uint32_t cnt)
{
	...
	*addr |= (cnt << 16) | (uint16_t) ((EPList[index].buf_ptr) >> 6);
	...
	{
#if defined(__GNUC__)
		((unaligned_uint32 *) dataptr)->value
				= ((unaligned_uint32 *) pData)->value;
#else
